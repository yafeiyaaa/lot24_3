...
def modify_event_df( events, position = np.nan, value = None, nums = [], result_stack = [], expression_stack = []):
    # 读取trial_info
    trial_info_df =pd.read_pickle('./Recording/' + session['subject'] + '.pkl')
    ...
    trial_info_df.to_pickle('./Recording/' + session['subject'] + '.pkl')
    return event_df
...
# 初始化session
def session_init(subject = None, nums = None ):
    '''trial info'''
    # 从题库中获得随机24点题目
    if(nums == None):
...
def create_recording(subject):
    ...
    data.to_pickle('./Recording/' + subject + '.pkl')
